...
class DensePoseOutput(object):
    def __init__(self, S, I, U, V, confidences, mask_thresh=0.5):
        """
        Args:
    ...
    def to_result(self, boxes_xywh):
        ...
        """
        result = DensePoseResult(boxes_xywh, self.S, self.I, self.U, self.V, self.mask_thresh)
        return result
    ...
    def __getitem__(self, item):
        ...
        return DensePoseOutput(S_selected, I_selected, U_selected, V_selected, conf_selected, self.mask_thresh)
...
class DensePoseResult(object):
    def __init__(self, boxes_xywh, S, I, U, V, mask_thresh=0.5):
        self.results = []
        ...
        for i, box_xywh in enumerate(boxes_xywh):
            result_i = self._output_to_result(box_xywh, S[[i]], I[[i]], U[[i]], V[[i]], mask_thresh)
    ...
    def _output_to_result(self, box_xywh, S, I, U, V, mask_thresh=0.5):
        ...
        s_bbox = F.interpolate(S, (h, w), mode="bilinear", align_corners=False)
        s_score_bbox = F.softmax(s_bbox,dim=1)
        s_bbox = s_bbox.argmax(1)
        ...
        if mask_thresh == 0.5:
            ...
            i_bbox = (
                    F.interpolate(I, (h, w), mode="bilinear", align_corners=False).argmax(dim=1)
                    * (s_score_bbox[:, 1] > mask_thresh).long()
            ).squeeze(0)
